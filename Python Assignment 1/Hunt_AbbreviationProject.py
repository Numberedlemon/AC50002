...
def getCharacterValues():
    values = open("values.txt", 'r') # opens the values file in read mode N.B.: THIS FILE MUST BE IN THE SAME DIRECTORY AS THE SOURCE FILE.
...
def getNames(filename):
    file = open(filename, "r") # open file with filename in write mode in the same dir as the source file. This overwrites the current files contents.
...
def letterScore(longName, letter):
    ...
    # splits the longname into a first and last name.
...
def createAbbrevs(fileName):
    ...
    file = open(fileName + "_output.txt", "w") # opens the output file in write mode in the same dir as the source file.
